...
func (s *Simulator) generateGGA(timestamp time.Time) string {
...
func (s *Simulator) generateNoFixGGA(timestamp time.Time) string {
...
func (s *Simulator) generateRMC(timestamp time.Time) string {
	timeStr := timestamp.UTC().Format("150405") // HHMMSS
...
func (s *Simulator) generateNoFixRMC(timestamp time.Time) string {
	timeStr := timestamp.UTC().Format("150405")
...
func (s *Simulator) generateGSA() string {
...
func (s *Simulator) generateGSV() []string {
...
func (s *Simulator) generateVTG() string {
...
func (s *Simulator) generateNoFixGLL(timestamp time.Time) string {
	utcTime := timestamp.UTC()
	timeStr := fmt.Sprintf("%02d%02d%02d.%02d",
		utcTime.Hour(), utcTime.Minute(), utcTime.Second(), utcTime.Nanosecond()/10000000) // HHMMSS.SS
...
func (s *Simulator) generateZDA(timestamp time.Time) string {
	utcTime := timestamp.UTC()
